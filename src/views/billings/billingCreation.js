...
const [jsonData, setJsonData] = useState({ title: '', details: [], total_amount: 0 });
...
const parseComparative = (rows, titleText) => {
 ...
 const parseDocxToJSON = async (file) => {
  ...
  if (!rows.length) return { title: titleText, details: [], total_amount: 0 }
  ...
  if (!result) return { title: titleText, details: [], total_amount: 0 }
